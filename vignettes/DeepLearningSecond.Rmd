...
trainingSCE <- trainingSCE[rowData(trainingSCE)$Type == "Gene Expression"]

testingLabels <- testingSCE[rowData(testingSCE)$ID == "CD45RA"] %>%
  SCEtoMatrix(.,transpose = TRUE)
testingSCE <- testingSCE[rowData(testingSCE)$Type == "Gene Expression"]
```
...
```{r featureSelect}
fit <- trendVar(trainingSCE, use.spikes = FALSE)
dec <- decomposeVar(trainingSCE, fit)
hvg <- dec$bio > 0 # save vector of genes
trainingSCE <- trainingSCE[hvg,]
testingSCE <- testingSCE[hvg,]
```

```{r separate}
trainingData <- SCEtoMatrix(trainingSCE,transpose = TRUE)
testingData <-  SCEtoMatrix(testingSCE,transpose = TRUE)
```
