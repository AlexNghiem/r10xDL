...
proteinGREP <- "CD.*|HLA-DR" #this GREP captures the proteins we are interested in from the SCE. Note that this does not include the IgG suite of proteins (these are controls)
...
getProteinMatrix <- function(x) { #from an SCE, gets the transposed matrix of proteins
  x[proteinList] %>%
    counts() %>%
    as.matrix() %>% #the data is originally a DelayedMatrix object, so we want to convert that to a regular matrix
    t() %>%
    clr() #we perform a centered log ratio transform. This is essentially just normalizing the protein counts
}
...
# 1: CD3, 2: CD19, 3: CD45RA, 4: CD4, 5: CD8a, 6: CD14, 7: CD45RO, 8: CD279_PD_1, 9: CD127, 10: CD197_CR7, 11: HLA_DR
...
output0 <- inputs %>%
  layer_dropout(rate = .25) %>% #first layer dropout rate set to .25 
  layer_dense(units = 64, activation = 'relu') %>% #each of these lines creates a new layer, and specifies the number of nodes and the activation function
...
model %>% compile(
  loss = "mse",
  optimizer = optimizer_rmsprop(lr = .0002), #learning rate here
...
scatters <- function() {
  par(mfrow = c(2,2))
  invisible(lapply(c(3,5,7,8), scatter))
}
