...
SCE <- read10xCounts("../10Xdata/5k_pbmc_protein_v3_nextgem_filtered_feature_bc_matrix.h5")

trainingSize = 5000 # how many cells to train on (this affects speed linearly.. 30000 is optimal)
testingSize = 527 # how many cells to test on (this does not affect speed much.. 5000 is plenty)
...
proteinList <- rowData(geneTrainingSCE)$Type == "Antibody Capture"
getProteinMatrix <- function(x) { # from an SCE, gets the transposed matrix of proteins
  x[rowData(geneTrainingSCE)$Type == "Antibody Capture"] %>%
    counts() %>%
    as.matrix() %>%
    t() %>%
...
scatters <- function() {
  par(mfrow = c(2,3))
  invisible(lapply(1:dim(trainingLabels)[2], scatter))
}

lossPlot <- function(x) {
  plot(1:epochs, unlist(history$metrics[names(history$metrics) == paste(colnames(trainingLabels)[x],"_mean_absolute_error", sep = "")]), ylab = paste("MAE of", colnames(trainingLabels)[x]), width = 2, height = 2)
}

lossPlots <- function() {
  par(mfrow = c(3,5))
  invisible(lapply(1:dim(trainingLabels)[2], lossPlot))
}
...
correlations <- function(){
  ...
  print(paste("The average correlation is", total/dim(trainingLabels)[2]))
}
```
