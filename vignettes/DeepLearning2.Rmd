...
trainingSCE <- read10xCounts("../10Xdata/5k_pbmc_protein_v3_nextgem_filtered_feature_bc_matrix.h5")
testingSCE <- read10xCounts("../10Xdata/5k_pbmc_protein_v3_filtered_feature_bc_matrix.h5")

trainingSize <- ncol(trainingSCE)
testingSize <- ncol(testingSCE)

#singler = CreateSinglerSeuratObject(counts = 'GSE74923_series_matrix.txt', annot = 'GSE74923_types.txt', project.name = 'GSE74923', min.genes = 500, min.cells = 2, npca = 10, regress.out ='nUMI', technology= 'C1', species = 'Mouse', citation = 'Kimmerling et al. 2016',reduce.file.size = T, variable.genes = 'de', normalize.gene.length = T)


# sum(rowData(trainingSCE) != rowData(testingSCE))
# This is a good sanity check -> the rows in each SCE are equivalent. This will not always be the case
