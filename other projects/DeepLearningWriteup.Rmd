---
title: "Deep Learning Workflow and Results"
output:
...
In this vignette, I will overview my deep learning explorations of the 10X genomics data published in July 2019. This data tracks around 10,000 PBMC cells. For each cell, the data include measurements for gene expression, protein expression, and antigen specificity.

Broadly, my goal was to create a machine learning approach to predict protein expression from gene expression data. This model, if it is effective, could be useful for predicting cell type, imputing sparse protein counts, or transferring to different scRNA-seq data sets to generate brand-new protein information. 

We have a lot of data here to work with, but it is not obvious how best to approach building the model. I decided to use a basic neural net, as they have shown excellent performance recently in transcriptomic problems, and are ideal for predicting the types of complex nonlinear functions I expect from these systems. Other options include decision forests and linear regression, both of which might work here and have relative benefits such as simplicity and readability.

Please note that several hyperparameters have been adjusted to help the vignette run quickly. I will mark these spots, and explain why other values may be more optimal.
...
## Input the data and break it down
...
proteinList <- (rowData(trainingSCE)$Type == "Antibody Capture") & !grepl("IgG*", rowData(trainingSCE)$Symbol)
...
# tensorflow complains when fed dashes, so we replace any in the protein names with underscores
...
proteinTrainingData <- trainingSCE %>%
  getProteinMatrix() %>%
  fixProteinColNames()
proteinTestingData <- testingSCE %>%
  getProteinMatrix() %>%
  fixProteinColNames()
...
# keep genes with at least 1 read in at least 1% of cells (remove low frequency genes) 
min_reads <- 1
...
Here is where we decide which genes and proteins to train on. I decided to train the model on only the more highly variable genes. Hopefully this reduces noise in the model and makes it easier and faster for the model to train. 
...
* Dropout - I will talk more about this in more detail later, as I did a more thorough analysis of it. Dropout is the process of choosing randomly some proportion of nodes from a layer (often the input layer) to get dropped completely. This helps ensure that the model is training using all available data, as sometimes key genes are not included. Using dropout does seem to have a large positive effect on performance. Good values seem to range from ~.2 to ~.5. 
...
history <- model %>% fit(
  x = geneTrainingData,
  y = lapply(1:dim(proteinTrainingData)[2], function(x) {proteinTrainingData[,x]}),
...
scatter <- function(x, y=".") {
  plot(proteinTestingData[,x], unlist(predict(model, geneTestingData)[x]), xlab = paste(colnames(proteinTestingData)[x],"log value from data"), ylab = "Log value predicted by model", pch = y)
}

scatters <- function() {
  par(mfrow = c(2,3))
  invisible(lapply(1:dim(proteinTrainingData)[2], scatter))
}

lossPlot <- function(x) {
  plot(1:epochs, unlist(history$metrics[names(history$metrics) == paste(colnames(proteinTrainingData)[x],"_mean_absolute_error", sep = "")]), ylab = paste("MAE of", colnames(proteinTrainingData)[x]))
}

lossPlots <- function() {
  par(mfrow = c(3,5))
  invisible(lapply(1:dim(proteinTrainingData)[2], lossPlot))
}

correlations <- function(){
  predictions <- predict(model, geneTestingData)
  total <- 0
  for (i in 1:dim(proteinTrainingData)[2]) {
    corr <- cor(as.numeric(unlist(predictions[i])), proteinTestingData[,i])
    if (!is.na(corr)){
      print(paste("The correlation between predicted and actual values for", colnames(proteinTestingData)[i], "is", corr))
      if (corr > 0) {
        total <- total + corr^2
      }
      else {
  ...
  print(paste("The average correlation is", total/dim(proteinTrainingData)[2]))
}
```
...
And finally, I printed out a simple list of the correlation between the model's predictions and the actual values. As you can see, many of the proteins are not able to be predicted at all by the model, while others are quite accurate.
